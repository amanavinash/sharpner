...
const addExpense = async (req, res, next) => {
  ...
  try {
    const data = await Expense.create({
      ...
      category,
      userId: req.user.id,
    }).then((expense) => { const totalExpenses = Number(req.user.totalExpenses) + Number(amount);
...
const getExpense = async (req, res, next) => {
  ...
  try {
    ...
    const NewData = await Expense.findAll({ offset: (page - 1) * limit, limit: limit,
    });
...
function uploadToS3(data, filename) {
  const IAM_USER_KEY = process.env.AWS_SECRET_KEY;
  const IAM_USER_SECRET = process.env.AWS_SECRET_PASSWORD;
